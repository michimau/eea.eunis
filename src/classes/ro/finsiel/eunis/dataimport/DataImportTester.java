	...
	*/
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (sessionManager.isAuthenticated() && sessionManager.isImportExportData_RIGHT()) {
	        ...
	        String SMTP_SERVER = request.getSession().getServletContext().getInitParameter("SMTP_SERVER");
	        String SMTP_USERNAME = request.getSession().getServletContext().getInitParameter("SMTP_USERNAME");
	        String SMTP_PASSWORD = request.getSession().getServletContext().getInitParameter("SMTP_PASSWORD");
	        String SMTP_SENDER = request.getSession().getServletContext().getInitParameter("SMTP_SENDER");
		            ...
		            if(!item.isFormField()){
		            	...
		            	if(runBackground){
		            		try{
		                		...
		                		JobDetail jobDetail = new JobDetail("testerJob", null, TesterJob.class);
		                		...
		                		jobDetail.getJobDataMap().put("smtpServer", SMTP_SERVER);
		                		jobDetail.getJobDataMap().put("smtpUsername", SMTP_USERNAME);
		                		jobDetail.getJobDataMap().put("smtpPassword", SMTP_PASSWORD);
		                		jobDetail.getJobDataMap().put("smtpSender", SMTP_SENDER);
		                				                		
		                		jobDetail.getJobDataMap().put("filePath", TEMP_DIR+"xmlFile");
		            		...
		            		errors = tester.test(TEMP_DIR+"/xmlFile", table, SQL_DRV, SQL_URL, SQL_USR, SQL_PWD);
		            	}
		            }
		    	}
		    	if(!runBackground){
