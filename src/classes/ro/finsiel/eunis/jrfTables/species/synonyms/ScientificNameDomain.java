...
public class ScientificNameDomain extends AbstractDomain implements Paginable {
  ...
  public void setup() {
    ...
    gSpecies = new JoinTable("CHM62EDT_GROUP_SPECIES E", "ID_GROUP_SPECIES", "ID_GROUP_SPECIES");
    gSpecies.addJoinColumn(new StringJoinColumn("COMMON_NAME", "grName", "setGrName"));
    this.addJoinTable(gSpecies);

    JoinTable Species2 = null;
    Species2 = new JoinTable("CHM62EDT_SPECIES D", "ID_SPECIES_LINK", "ID_SPECIES_LINK");
    Species2.addJoinColumn(new StringJoinColumn("SCIENTIFIC_NAME", "scName", "setScName"));
    Species2.addJoinColumn(new IntegerJoinColumn("ID_SPECIES", "IdSpec", "setIdSpec"));
    Species2.addJoinColumn(new IntegerJoinColumn("ID_SPECIES_LINK", "IdSpecLink", "setIdSpecLink"));
    Species2.addJoinColumn(new IntegerJoinColumn("ID_NATURE_OBJECT", "IdNatObj", "setIdNatObj"));
...
//    filterSQL.append(Utilities.showEUNISInvalidatedSpecies("AND D.VALID_NAME", showEUNISInvalidatedSpecies));
    ...
    List tempList = this.findWhere(filterSQL.toString());
     for (int i = 0; i < tempList.size(); i++) {
      ScientificNamePersist specie = (ScientificNamePersist) tempList.get(i);
    ...
    try {
      ...
      if (sortCriteria.length > 0) {
        ...
        do {
          ...
          SynonymsSortCriteria criteria = (SynonymsSortCriteria) sortCriteria[i]; // Notice the upcast here
