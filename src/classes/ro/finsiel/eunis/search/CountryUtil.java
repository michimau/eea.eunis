...
public class CountryUtil {
  ...
  public static Iterator findRegionsFromCountry(String countryCode) {
    ...
    if (countryCode.equals("any")) {
      ...
      try
      {
        _regionsCodes = new Chm62edtCountryBiogeoregionDomain().findWhere("CODE_COUNTRY='" + countryCode + "' AND CODE_BIOGEOREGION<>'nd'");
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
    ...
    while (_it.hasNext())
    {
      ...
      _regions.addElement(new RegionWrapper(regionCode2Name(_aRegion.getCodeBiogeoregion()), _aRegion.getCodeBiogeoregion(), "idGeoscope", _aRegion.getPercentage()));
    }
  ...
  {
    if (null == regionCode) return "null";
    String result = "";
  ...
  public String findRegionIdGeoscope(String regionName) throws RecordNotFoundException {
    if (regionName.equals("any")) return "any";
  ...
  public String findCountryIdGeoscope(String countryName) throws RecordNotFoundException {
    if (countryName.equals("any")) return "any";
    Integer ret = new Integer(-1);
    List _list = new Vector();
    try
    {
    ...
    if (!_list.isEmpty())
    {
      ...
      throw new RecordNotFoundException("findCountryIdGeoscope(String): No country with name " + countryName + " has been found.");
    }
    return ret.toString();
  }
  ...
  public static List findAllCountries() {
    ...
    try
    {
      ret = new Chm62edtCountryDomain().findWhereOrderBy("ISO_2L<>'' AND ISO_2L<>'null' AND ISO_2L IS NOT NULL AND SELECTION <> 0", "AREA_NAME_EN");
    } catch (Exception _ex) {
      _ex.printStackTrace(System.err);
      ret = new Vector();
    }
  ...
  public static String areaNameEn2ISO2L(String areaNameEn) {
    String result = null;
    if (null == areaNameEn) return result;
    try
    {
  ...
  public static String getIso3LByCountryName(String name) {
    ...
    try
    {
      ...
      if (country != null && country.size() > 0) {
        ISO = (((Chm62edtCountryPersist) country.get(0)).getIso3l() == null ? "" : ((Chm62edtCountryPersist) country.get(0)).getIso3l());
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public static String findBiogeoregionByIDGeoscope(Integer idGeoscope)
  {
    ...
    public static String getNOListString(List NOList,String alias) {
        if(NOList == null || NOList.size() <= 0) return "";
        ...
        for(int i=0;i<NOList.size();i++) {
            ...
            result.append("'");
            if(i < NOList.size() - 1) result.append(",");
            else result.append(" ) ");
        }
     ...
     sql =  " select distinct a.id_nature_object FROM CHM62EDT_HABITAT AS A " +
      ...
      if(null != country && country.trim().length() > 0)
           sql += " INNER JOIN CHM62EDT_COUNTRY AS C ON B.ID_GEOSCOPE = C.ID_GEOSCOPE " +
            (null != region && region.trim().length() > 0 ? "INNER" : "LEFT OUTER") + " JOIN CHM62EDT_BIOGEOREGION AS D ON B.ID_GEOSCOPE_LINK = D.ID_GEOSCOPE where " + whereCond;
      else
