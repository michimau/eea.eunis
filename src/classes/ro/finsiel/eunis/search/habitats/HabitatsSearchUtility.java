...
public class HabitatsSearchUtility {
  ...
  public static List findHabitatsWithCriteria(String name, Integer relationOp, Integer database,
    ...
    if (null == name || null == relationOp) return new Vector();
    if (useScientific == false && useVernacular == false && useDescription == false) return new Vector();
    ...
    if (useVernacular) {
      if (useScientific) whereSQL.append(" OR ");
      whereSQL.append(Utilities.prepareSQLOperator("A.DESCRIPTION", name, relationOp));
      whereSQL.append(" ");
    }
    if (useDescription) {
      if (useScientific || useVernacular) whereSQL.append(" OR ");
  ...
  public static List findEUNISHabitatTypes() {
    List results = new Vector();
    try {
      results = new Chm62edtHabitatDomain().findWhereOrderBy("LEVEL=1 and ID_HABITAT>=1 and ID_HABITAT < 10000", "EUNIS_HABITAT_CODE");
  ...
  public static List findDatabaseClassificationsExamples(int idClassification, String codePartSQL, int database) {
    ...
    if (-1 == idClassification) // Current classification
    {
      ...
      if (database != CodeDomain.SEARCH_BOTH.intValue()) {
        ...
        sql += " AND ((EUNIS_HABITAT_CODE " + codePartSQL + " AND EUNIS_HABITAT_CODE <> '0' AND EUNIS_HABITAT_CODE IS NOT NULL AND TRIM(EUNIS_HABITAT_CODE)<>'') " +
                " OR (CODE_ANNEX1 " + codePartSQL + " AND CODE_ANNEX1 <> '0' AND CODE_ANNEX1 IS NOT NULL AND TRIM(CODE_ANNEX1)<>'')) ";
        sql += " AND ID_HABITAT <> '-1' AND ID_HABITAT <> '10000' ";
      ...
      if (database != CodeDomain.SEARCH_BOTH.intValue()) {
        ...
        sql += " AND ((A.EUNIS_HABITAT_CODE " + codePartSQL + " AND A.EUNIS_HABITAT_CODE <> '0' AND A.EUNIS_HABITAT_CODE IS NOT NULL AND TRIM(A.EUNIS_HABITAT_CODE)<>'') " +
                " OR (A.CODE_ANNEX1 " + codePartSQL + " AND A.CODE_ANNEX1 <> '0' AND A.CODE_ANNEX1 IS NOT NULL AND TRIM(A.CODE_ANNEX1)<>'')" +
                " OR (B.CODE " + codePartSQL + "  AND B.CODE IS NOT NULL AND TRIM(B.CODE)<>''))";
        sql += " AND A.ID_HABITAT <> '-1' AND A.ID_HABITAT <> '10000' ";
      }
  ...
  public static String getHabitatLevelName(String habCode) {
    if (null == habCode) return "n/a";
    if (habCode.equalsIgnoreCase("any")) return " all habitats ";
    String ret = "";
    try {
  ...
  public static List findOtherClassifications(int habitatID, Integer relationOp, String searchString) {
    Vector result = new Vector();
    // Find in HABITAT_CLASS_CODE WHERE ID_HABITAT = habitatID
    List list = new Chm62edtHabitatClassCodeDomain().findWhere("ID_HABITAT=" + habitatID + " AND " + Utilities.prepareSQLOperator("CODE", searchString, relationOp));
    Iterator it = list.iterator();
    while (it.hasNext()) {
      Chm62edtHabitatClassCodePersist aHabitat = (Chm62edtHabitatClassCodePersist) it.next();
      String relationType = (null == aHabitat.getRelationType() || "" == aHabitat.getRelationType()) ? "not specified" : aHabitat.getRelationType();
  ...
  public static String findClassificationName(String idClassification) {
    ...
    try
    {
      ...
      if (null != results && results.size() > 0) name = (((Chm62edtClassCodePersist) results.get(0)).getClassName() == null ? "" : ((Chm62edtClassCodePersist) results.get(0)).getClassName());
    } catch (Exception ex) {
      ex.printStackTrace(System.err);
    }
    return name;
  ...
  public static String findHabitatNameByEunisCode(String eunisCode) {
    ...
    try
    {
      ...
      if (null != results && results.size() > 0) name = (((Chm62edtHabitatPersist) results.get(0)).getScientificName() == null ? "" : ((Chm62edtHabitatPersist) results.get(0)).getScientificName());
    } catch (Exception ex) {
      ex.printStackTrace(System.err);
    }
    return name;
  ...
  public static String findHabitatNameById(String habId) {
    ...
    try
    {
      ...
      if (null != results && results.size() > 0)
        name = (((Chm62edtHabitatPersist) results.get(0)).getScientificName() == null ? "" : ((Chm62edtHabitatPersist) results.get(0)).getScientificName());
    } catch (Exception ex) {
      ex.printStackTrace(System.err);
    }
    return name;
  ...
  public static int findCurrentClassificationID(String classification) {
    ...
    if (-1 == ret) {
      System.out.println("findCurrentClassification(" + classification + ") = -1. It is possible that classification code is incorrect.");
    }
    return ret;
  }
