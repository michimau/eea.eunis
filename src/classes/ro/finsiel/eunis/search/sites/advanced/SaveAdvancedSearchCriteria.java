...
public class SaveAdvancedSearchCriteria {
  ...
  public boolean SaveCriteria() {
    ...
    try {
      ...
      long maxIdNode = MaxIdNode(name, natureObject);
  ...
  public static String getSourceDB(String criteria,
                                   ...
                                   String SQL_PWD) {
    String sourceDB = "";
    long maxIdNode = MaxIdNode(criteria, natureObject, SQL_DRV, SQL_URL, SQL_USR, SQL_PWD);
   ...
   * @return max value.
   */
  private long MaxIdNode(String criteria, String natureObject) {
    String SQL = "";
    Connection con = null;
    Statement ps = null;
    ResultSet rs = null;
    ...
    try {
      ...
      if (rs.isBeforeFirst()) {
        ...
        while (!rs.isLast()) {
          ...
          result += "<td><a title=\"Load criteria\" href=\"javascript:setFormDeleteSaveCriteria('" + fromWhere + "','" + rs.getString("CRITERIA_NAME") + "','" + rs.getString("NATURE_OBJECT") + "');\"><img alt=\"Delete\" src=\"images/mini/delete.jpg\" border=\"0\" align=\"absmiddle\"></a>&nbsp;&nbsp;<a title=\"Load criteria\" href=\"javascript:setFormLoadSaveCriteria('" + fromWhere + "','" + rs.getString("CRITERIA_NAME") + "','" + rs.getString("NATURE_OBJECT") + "','" + sourceDB + "');\">" + rs.getString("DESCRIPTION") + "</a></td>";
          result += "</tr>";
        }
        result += "</table>";
      }
    ...
    try {
      ...
      if (rs.isBeforeFirst()) {
        while (!rs.isLast()) {
          rs.next();
          //is not SourceDB
          if (!rs.getString("ID_NODE").equalsIgnoreCase(new Long(MaxIdNode(rs.getString("criteria_name"), rs.getString("nature_object"), SQL_DRV, SQL_URL, SQL_USR, SQL_PWD)).toString())) {
            Connection con1 = DriverManager.getConnection(SQL_URL, SQL_USR, SQL_PWD);
            Statement ps1 = con1.createStatement();
