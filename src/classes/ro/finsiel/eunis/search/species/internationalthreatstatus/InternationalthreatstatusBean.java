...
public class InternationalthreatstatusBean extends AbstractFormBean {
  ...
  public AbstractSearchCriteria[] toSearchCriteria() {
    ...
    if (null != criteriaSearch && null != criteriaType && null != oper) {
      for (int i = 0; i < criteriaSearch.length; i++) {
        Integer _criteriaType = Utilities.checkedStringToInt(criteriaType[i], InternationalthreatstatusSearchCriteria.CRITERIA_SCIENTIFIC_NAME);
    ...
    for (int i = 0; i < ret.length; i++) ret[i] = (InternationalthreatstatusSearchCriteria) criterias.get(i);
    return ret;
  }
  ...
  public String getStringMain() {
    ...
    if (idGroup != null && idCountry != null && idConservation != null) {
      ...
      if(namecountry != null && namecountry.trim().length() > 0) result += " and area '" + namecountry + "'";
    }
    return result;
  }
  ...
  public String toFORMParam(Vector classFields) {
    ...
    if (null != idGroup) form.append(Utilities.writeFormParameter("idGroup", idGroup));
    if (null != idCountry) form.append(Utilities.writeFormParameter("idCountry", idCountry));
    if (null != idConservation) form.append(Utilities.writeFormParameter("idConservation", idConservation));
    ...
    if (null != showGeo && showGeo.equalsIgnoreCase("true")) form.append(Utilities.writeFormParameter("showGeo", AbstractFormBean.SHOW.toString()));
