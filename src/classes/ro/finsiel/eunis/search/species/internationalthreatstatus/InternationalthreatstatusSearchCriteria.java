...
public class InternationalthreatstatusSearchCriteria extends AbstractSearchCriteria {
  ...
  public String toSQL() {
    ...
    if (null != idGroup && null != idCountry && null != idConservation) {
      if ((idGroup.intValue() != -1) && (idConservation.intValue() != -1)) sql.append("F.ID_COUNTRY = "+idCountry+" AND C.ID_GROUP_SPECIES=" + idGroup + " AND H.ID_CONSERVATION_STATUS=" + idConservation);
      if ((idGroup.intValue() == -1) && (idConservation.intValue() != -1)) sql.append("F.ID_COUNTRY = "+idCountry+" AND H.ID_CONSERVATION_STATUS=" + idConservation);
      if ((idGroup.intValue() != -1) && (idConservation.intValue() == -1)) sql.append(" F.ID_COUNTRY = "+idCountry+" AND C.ID_GROUP_SPECIES=" + idGroup);
      if ((idGroup.intValue() == -1) && (idConservation.intValue() == -1)) sql.append(" F.ID_COUNTRY = "+idCountry);
    }
