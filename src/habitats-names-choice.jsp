...
<head>
  ...
  %>
  <title>
    <%=contentManagement.getContent("habitats_names-choice_title", false)%>
  </title>
...
<u><%=contentManagement.getContent("habitats_names-choice_01")%></u>
...
%>
<strong>
  <%=contentManagement.getContent("habitats_names-choice_02")%>
</strong>
<br />
<br />
<%
  ...
  <table summary="List of values" border="1" cellpadding="2" cellspacing="0" style="border-collapse: collapse" width="100%">
    ...
    <tr>
      <td bgcolor="<%=(0 == (i++ % 2)) ? "#EEEEEE" : "#FFFFFF"%>">
        <a title="Click link to select the value" href="javascript:setLine('<%=Utilities.treatURLSpecialCharacters(habitat.getScientificName())%>');"><%=habitat.getScientificName()%></a>
      </td>
    </tr>
    <%
      }
...
<form action="">
  ...
  <input title="Close window" type="button" value="<%=contentManagement.getContent("habitats_names-choice_03", false )%>" onclick="javascript:window.close()" name="button" id="button" class="inputTextField" />
</form>
</body>
</html>
