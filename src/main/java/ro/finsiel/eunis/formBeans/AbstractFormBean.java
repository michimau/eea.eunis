...
public abstract class AbstractFormBean implements java.io.Serializable {
  ...
  public void prepareFilterCriterias() {
    ...
    {
      int removeFilterIndex = Utilities.checkedStringToInt(getRemoveFilterIndex(), -1);
      if (removeFilterIndex - 1 >= 0 && removeFilterIndex - 1 < getCriteriaSearch().length)
      {
  ...
  public final Integer changeAscendency(AbstractSortCriteria criteria, boolean alter) {
    ...
    {
      if (0 == AbstractSortCriteria.ASCENDENCY_ASC.compareTo(criteria.getAscendency()))
      {
        return AbstractSortCriteria.ASCENDENCY_DESC;
      }
      if (0 == AbstractSortCriteria.ASCENDENCY_DESC.compareTo(criteria.getAscendency()))
      {
        return AbstractSortCriteria.ASCENDENCY_NONE;
      }
      if (0 == AbstractSortCriteria.ASCENDENCY_NONE.compareTo(criteria.getAscendency()))
      {
        return AbstractSortCriteria.ASCENDENCY_ASC;
      }
    }
    if (null == criteria) return AbstractSortCriteria.ASCENDENCY_ASC;
  ...
  protected StringBuffer toFORMParamSuper(Vector classFields)
  {
    ...
    if (classFields.contains("pageSize") && null != pageSize) {
      formString.append(Utilities.writeFormParameter("pageSize", "" + pageSize));
    }
    if (classFields.contains("currentPage") && null != currentPage) {
      formString.append(Utilities.writeFormParameter("currentPage", "" + currentPage));
    }
    if (classFields.contains("sort") && null != sort) {
  ...
  public void setCriteriaSearch(String[] criteriaSearch) {
    this.criteriaSearch = (null != criteriaSearch) ? Utilities.trimArray(criteriaSearch) : criteriaSearch;
  }
