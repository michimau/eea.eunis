...
public class InternationalThreatStatusDomain extends AbstractDomain implements Paginable {
  ...
  public void setup() {
    ...
    this.addColumnSpec(new IntegerColumnSpec("ID_GROUP_SPECIES", "getIdGroupspecies", "setIdGroupspecies", DEFAULT_TO_ZERO, NATURAL_PRIMARY_KEY));
    this.addColumnSpec(new StringColumnSpec("COMMON_NAME", "getCommonName", "setCommonName", DEFAULT_TO_EMPTY_STRING, REQUIRED));
    ...
    this.addColumnSpec(new ShortColumnSpec("SELECTION", "getSelection", "setSelection", null));
    this.addColumnSpec(new IntegerColumnSpec("ID_DC", "getIdDc", "setIdDc", DEFAULT_TO_NULL));

    // Joined tables
    JoinTable species = null;
    species = new JoinTable("CHM62EDT_SPECIES C", "ID_GROUP_SPECIES", "ID_GROUP_SPECIES");
    ...
    species.addJoinColumn(new StringJoinColumn("SCIENTIFIC_NAME", "scName", "setScName"));
    species.addJoinColumn(new IntegerJoinColumn("ID_NATURE_OBJECT", "IdNatObj", "setIdNatureObject"));
    this.addJoinTable(species);

    JoinTable reports = null;
    reports = new JoinTable("CHM62EDT_REPORTS E", "ID_NATURE_OBJECT", "ID_NATURE_OBJECT");
    species.addJoinTable(reports);

    JoinTable country = null;
    country = new JoinTable("CHM62EDT_COUNTRY F", "ID_GEOSCOPE", "ID_GEOSCOPE");
    country.addJoinColumn(new StringJoinColumn("AREA_NAME_EN", "areaNameEn", "setAreaNameEn"));
    country.addJoinColumn(new IntegerJoinColumn("ID_COUNTRY", "idCountry", "setIdCountry"));
    reports.addJoinTable(country);

    JoinTable reporttype = null;
    reporttype = new JoinTable("CHM62EDT_REPORT_TYPE G", "ID_REPORT_TYPE", "ID_REPORT_TYPE");
    reports.addJoinTable(reporttype);

    JoinTable status = null;
    status = new JoinTable("CHM62EDT_CONSERVATION_STATUS H", "ID_LOOKUP", "ID_CONSERVATION_STATUS");
    status.addJoinColumn(new StringJoinColumn("NAME", "defAbrev", "setDefAbrev"));
    status.addJoinColumn(new IntegerJoinColumn("ID_CONSERVATION_STATUS", "idCons", "setIdCons"));
    status.addJoinColumn(new IntegerJoinColumn("ID_DC", "iddc", "setIdDCore"));
    reporttype.addJoinTable(status);

    OuterJoinTable family = null;
    family = new OuterJoinTable("CHM62EDT_TAXONOMY I", "ID_TAXONOMY", "ID_TAXONOMY");
    family.addJoinColumn(new StringJoinColumn("NAME", "taxonomyName", "setTaxonomyName"));
    family.addJoinColumn(new StringJoinColumn("LEVEL", "taxonomyLevel", "setTaxonomyLevel"));
    family.addJoinColumn(new StringJoinColumn("TAXONOMY_TREE", "taxonomyTree", "setTaxonomyTree"));
    family.addJoinColumn(new StringJoinColumn("NAME", "taxonomicNameFamily", "setTaxonomicNameFamily"));
    family.addJoinColumn(new StringJoinColumn("NAME", "taxonomicNameOrder", "setTaxonomicNameOrder"));
...
//    family.addJoinTable(order);
  ...
  private StringBuffer _prepareWhereSearch() throws CriteriaMissingException {
    ...
    filterSQL.append(" trim(F.AREA_NAME_EN) not like 'ospar%' and (F.ISO_2L is null or trim(F.ISO_2L) = '') AND G.LOOKUP_TYPE = 'CONSERVATION_STATUS' ");
    ...
    try {
      ...
      if (sortCriteria.length > 0) {
        ...
        do {
          ...
          InternationalthreatstatusSortCriteria criteria = (InternationalthreatstatusSortCriteria) sortCriteria[i]; // Notice the upcast here
