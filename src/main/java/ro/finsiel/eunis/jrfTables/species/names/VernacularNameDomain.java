...
public class VernacularNameDomain extends AbstractDomain implements Paginable {
  ...
  public void setup() {
    ...
    reportType = new JoinTable("CHM62EDT_REPORT_TYPE F", "ID_REPORT_TYPE", "ID_REPORT_TYPE");
    this.addJoinTable(reportType);

    JoinTable language = null;
    language = new JoinTable("CHM62EDT_LANGUAGE H", "ID_LOOKUP", "ID_LANGUAGE");
    reportType.addJoinTable(language);

    JoinTable attributes = null;
    attributes = new JoinTable("CHM62EDT_REPORT_ATTRIBUTES I", "ID_REPORT_ATTRIBUTES", "ID_REPORT_ATTRIBUTES");
    attributes.addJoinColumn(new StringJoinColumn("VALUE", "value", "setValue"));
    this.addJoinTable(attributes);

    JoinTable species = null;
    species = new JoinTable("CHM62EDT_SPECIES A", "ID_NATURE_OBJECT", "ID_NATURE_OBJECT");
    species.addJoinColumn(new StringJoinColumn("SCIENTIFIC_NAME", "scientificName", "setScientificName"));
    species.addJoinColumn(new ShortJoinColumn("VALID_NAME", "validName", "setValidName"));
...
//    taxCodeFamily.addJoinTable(taxCodeOrder);
  ...
  private Long _rawCount() throws CriteriaMissingException {
    ...
    // Set the main QUERY
    sql.append("SELECT COUNT(DISTINCT A.ID_NATURE_OBJECT) FROM CHM62EDT_REPORTS AS G " +
