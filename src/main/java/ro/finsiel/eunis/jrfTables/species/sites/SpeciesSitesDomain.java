...
public class SpeciesSitesDomain extends AbstractDomain implements Paginable {
    ...
    try {
      ...
      if (sortCriteria.length > 0) {
        ...
        do {
          ...
          SitesSortCriteria criteria = (SitesSortCriteria) sortCriteria[i]; // Notice the upcast here
  ...
  public List findSitesWithSpecies(SitesSearchCriteria criteria,
    ...
    String sql = prepareSQL("H.ID_SPECIES, H.ID_NATURE_OBJECT, H.SCIENTIFIC_NAME, H.ID_SPECIES_LINK, " +
            ...
            filterSQL + " GROUP BY C.ID_NATURE_OBJECT ORDER BY C.NAME", true);
    try {
  ...
  public List findPopupLOV(SitesSearchCriteria criteria,
                           ...
                           boolean showEUNISInvalidatedSpecies) {
    if (null == criteria) {
      System.out.println("Warning: " + SpeciesSitesDomain.class.getName() + "::findPopupLOV(" + criteria + ", " + "...). One of criterias was null.");
      return new Vector();
    }
    this.searchCriteria = new AbstractSearchCriteria[1];
    this.searchCriteria[0] = criteria;
    ...
    if (searchAttribute.intValue() == SitesSearchCriteria.SEARCH_NAME.intValue()) {
      sql = prepareSQL("H.ID_SPECIES, H.ID_NATURE_OBJECT, H.SCIENTIFIC_NAME, H.ID_SPECIES_LINK, " +
              "C.NAME, C.NAME, C.NAME, C.NAME, C.NAME, C.NAME, C.NAME,C.SOURCE_DB ",
              filterSQL + " GROUP BY C.ID_NATURE_OBJECT ORDER BY C.NAME LIMIT 0," + Utilities.MAX_POPUP_RESULTS, true);
      try {
...
//              " LEFT JOIN CHM62EDT_TAXONOMY AS L ON (J.ID_TAXONOMY_LINK = L.ID_TAXONOMY )";
    ...
    if (searchAttribute.intValue() == SitesSearchCriteria.SEARCH_NAME.intValue()) {
      sql = "SELECT " + what +
              ...
              joinWithGroupSpeciesAndTaxonomy +
