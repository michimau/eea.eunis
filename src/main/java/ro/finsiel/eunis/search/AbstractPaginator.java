...
/**
 * This is the paginator class used for all the results from search page.
 * Keep the track of currently displayed page on browser, page size. It has its data retrieved from Data factory
 * classes represented by JRF Domain classes. All data factories must implement the Paginable interface.
 * In pages with results declare an object derived from this class depending on the type of search.<br />
...
public abstract class AbstractPaginator {
   ...
   * @param finder Provide an finder object in order to do all the initializations. Usually this is derived from JRF...
   */
   ...
   * @param newSize New number of results displayed per page
   */
  public final void setPageSize(final int newSize) {
    if (newSize <= 0) pageSize = DEFAULT_PAGE_SIZE;
   ...
   * @throws InitializationException If the finder object was null.
   * @throws CriteriaMissingException If you didn't initialize correcly (with setSearchCriteria...) the finder.
   */
  public List getPage(int pageNumber) throws InitializationException, CriteriaMissingException {
    if (null == finder) throw new InitializationException("'finder' object was null, so call 'init(finder)' from the finder object failed.");
   ...
   * @throws CriteriaMissingException When search is not initialized correctly.
   */
   ...
   * @throws CriteriaMissingException When search is not initialized correcly.
   */
  ...
  /**
   * Setter for currentPage property. Also performs bounds checking (you can't specify a page index > countPages(). If
   * do currentPage = countPages() - 1. You can't also specify a page index  < 0. If do, currentPage = 0.
   * @param currentPage New value for currentPage.
   * @return The real current page (normally should be currentPage, but if invalid page index specified, this value is
  ...
  public final int setCurrentPage(int currentPage) {
    try
    {
      ...
      System.err.println("Exception while setting currentPage, could not perform the search to count results. No criteria set.");
