  ...
  */
  public int maxLevel(String codParent) {
    ...
    if (lHabitats != null) {
      ...
      while (hit.hasNext()) {
        Chm62edtHabitatPersist hh = (Chm62edtHabitatPersist) hit.next();
        if (max < hh.getHabLevel().intValue()) max = hh.getHabLevel().intValue();
      }
    }
    return max;
  }
  ...
  public int findLevel(String code) {
    if (code != null) {
      ...
      if (l != null && l.size() > 0) {
        return (((Chm62edtHabitatPersist) l.get(0)).getHabLevel() != null ? ((Chm62edtHabitatPersist) l.get(0)).getHabLevel().intValue() : -1);
      } else
        return -1;
    } else
      return -1;
  ...
  private List getListResults(int level, String codParent, int no) {
    ...
    while (wasFind == false && levelChild <= 3) {
      List l1 = new Chm62edtHabitatDomain().findWhere("ID_HABITAT>10000 AND CODE_2000 <> '" + codParent + "' AND CODE_2000 LIKE '" + codParent.substring(0, no) + "%' AND LEVEL=" + levelChild);
