...
public class ReferencesForHabitats {
  ...
  public void setReferencesList(String fromWhere, boolean useLimit) {
    ...
    if (sql.length() > 0) haveWhere = true;
    ...
    if (fromWhere.equalsIgnoreCase("author")) sql.append(" GROUP BY D.SOURCE ");
    if (fromWhere.equalsIgnoreCase("date")) sql.append(" GROUP BY E.CREATED ");
    if (fromWhere.equalsIgnoreCase("title")) sql.append(" GROUP BY F.TITLE ");
    if (fromWhere.equalsIgnoreCase("editor")) sql.append(" GROUP BY D.EDITOR ");
    if (fromWhere.equalsIgnoreCase("publisher")) sql.append(" GROUP BY G.PUBLISHER ");

    if (useLimit) sql.append(" LIMIT 0," + Utilities.MAX_POPUP_RESULTS);
    ...
    if (haveWhere)
      ...
      if (fromWhere.equalsIgnoreCase("author")) result_list = new DcSourceHabitatDomain().findWhere("1=1 " + sql.toString());
      if (fromWhere.equalsIgnoreCase("date")) result_list = new DcDateHabitatDomain().findWhere("1=1 " + sql.toString());
      if (fromWhere.equalsIgnoreCase("title")) result_list = new DcTitleHabitatDomain().findWhere("1=1 " + sql.toString());
      if (fromWhere.equalsIgnoreCase("editor")) result_list = new DcSourceHabitatDomain().findWhere("1=1 " + sql.toString());
      if (fromWhere.equalsIgnoreCase("publisher")) result_list = new DcPublisherHabitatDomain().findWhere("1=1 " + sql.toString());
