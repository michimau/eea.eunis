...
public class NeighborhoodSearchCriteria extends SitesSearchCriteria {
  ...
  public String toSQL() {
    ...
    if (null != englishName && null != relationOp) {
      ...
      if (null != yearMin) {
        ...
        sql.append(" AND (CONCAT(IF(A.source_db='BIOGENETIC',left(designation_date,4),''), " +
  ...
  public String toHumanString() {
    ...
    if (null != criteriaSearch && null != criteriaType && null != oper) {
      if (0 == criteriaType.compareTo(CRITERIA_SOURCE_DB)) {
        ret.append(Utilities.prepareHumanString((String) humanMappings.get(criteriaType), SitesSearchUtility.translateSourceDB(criteriaSearch), oper));
      } else {
