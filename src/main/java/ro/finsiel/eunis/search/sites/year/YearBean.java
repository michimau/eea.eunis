...
public class YearBean extends SitesFormBean {
  ...
  public AbstractSearchCriteria getMainSearchCriteria() {
    ...
    if (null != searchString || (null != searchStringMin && null != searchStringMax)) {
      ...
      criteria = new YearSearchCriteria(relationOp, searchString,
              searchStringMin, searchStringMax, country);
    }
    return criteria;
  }
  ...
  public AbstractSearchCriteria[] toSearchCriteria() {
    ...
    if (null != searchString || (null != searchStringMin && null != searchStringMax)) {
      ...
      Integer relationOp = Utilities.checkedStringToInt(this.relationOp, Utilities.OPERATOR_CONTAINS);
      YearSearchCriteria _criteria = new YearSearchCriteria(relationOp, searchString,
              searchStringMin, searchStringMax, country);
    ...
    for (int i = 0; i < ret.length; i++) ret[i] = (YearSearchCriteria) criterias.get(i);
  ...
  public AbstractSortCriteria[] toSortCriteria() {
    ...
    for (int i = 0; i < sort.length; i++) {
      YearSortCriteria criteria = new YearSortCriteria(
              Utilities.checkedStringToInt(sort[i], YearSortCriteria.ASCENDENCY_NONE),
