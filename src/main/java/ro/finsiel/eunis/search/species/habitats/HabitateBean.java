...
/**
 * Form bean used for species->habitats.
 * @author finsiel.
 */
public class HabitateBean extends AbstractFormBean {
  ...
  public AbstractSearchCriteria getMainSearchCriteria() {
    ...
    return new HabitateSearchCriteria(_searchAttribute, this.scientificName, relationOp);
  }
  ...
  public AbstractSearchCriteria[] toSearchCriteria() {
    ...
    if (null != criteriaSearch && null != criteriaType && null != oper) {
      for (int i = 0; i < criteriaSearch.length; i++) {
        Integer _criteriaType = Utilities.checkedStringToInt(criteriaType[i], HabitateSearchCriteria.CRITERIA_SCIENTIFIC_NAME);
        Integer _oper = Utilities.checkedStringToInt(oper[i], Utilities.OPERATOR_CONTAINS);
        criterias.addElement(new HabitateSearchCriteria(criteriaSearch[i], _criteriaType, _oper));
      }
    }
    HabitateSearchCriteria[] ret = new HabitateSearchCriteria[criterias.size()];
    for (int i = 0; i < ret.length; i++) ret[i] = (HabitateSearchCriteria) criterias.get(i);
  ...
  public AbstractSortCriteria[] toSortCriteria() {
    ...
    for (int i = 0; i < sort.length; i++) {
      HabitateSortCriteria criteria = new HabitateSortCriteria(
              Utilities.checkedStringToInt(sort[i], HabitateSortCriteria.ASCENDENCY_NONE),
              Utilities.checkedStringToInt(ascendency[i], HabitateSortCriteria.ASCENDENCY_NONE));
