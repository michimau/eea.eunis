    ...
    if (null != groupName && null != scientificName) {
      if (0 == typeForm.compareTo(CRITERIA_SPECIES)) {
        ...
        if (groupName.equalsIgnoreCase("any")) {
          ...
          result.append(Utilities.prepareSQLOperator((String) sqlMappings.get(CRITERIA_SPECIES), scientificName, Utilities.OPERATOR_CONTAINS));
        } else {
          ...
          result.append("E.SCIENTIFIC_NAME LIKE '%" + scientificName + "%' AND E.ID_GROUP_SPECIES = '" + groupName + "' AND B.LOOKUP_TYPE = 'LEGAL_STATUS'");
        }
      }
    }
    // form 2
...
//    System.out.println("annex = " + annex);
      ...
      if (0 == typeForm.compareTo(CRITERIA_LEGAL)) {
        if (!legalText.equalsIgnoreCase("any")) {
          if (groupName.equalsIgnoreCase("any")) {
            ...
            // second case (not 'Any group')
            result.append("ANNEX='" + annex + "' AND LOOKUP_TYPE='LEGAL_STATUS' AND ALTERNATIVE='" + legalText + "' AND E.ID_GROUP_SPECIES='" + groupName + "'");
          }
        } else {
          if (groupName.equalsIgnoreCase("any")) {
            // first case ('Any group')
    ...
    if (null != criteriaSearch && null != criteriaType && null != oper && null != typeForm) {
      ...
      if (0 == criteriaType.compareTo(CRITERIA_GROUP)) transCriteria = SpeciesSearchUtility.findGroupID(criteriaSearch).toString();
  ...
  public String toHumanString() {
    ...
    if (null != groupName && null != scientificName) {
      ...
      if (0 == typeForm.compareTo(CRITERIA_SPECIES)) {
        // first case ('Any group')
        result.append(Utilities.prepareHumanString((String) humanMappings.get(CRITERIA_SPECIES), scientificName, Utilities.OPERATOR_CONTAINS));
      }
    }
    // form 2
    if (null != groupName && null != legalText) {
