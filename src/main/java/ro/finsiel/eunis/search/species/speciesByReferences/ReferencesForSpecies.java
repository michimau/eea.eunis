...
public class ReferencesForSpecies {
    ...
    if (useLimit) {
      if (resultList.size() > 0) {
        if (Utilities.MAX_POPUP_RESULTS < resultList.size()) {
          for (int i = 0; i < resultList.size(); i++) results.addElement(resultList.get(i));
        } else {
          results = resultList;
        }
      }
  ...
  private Vector returnReferences(String sql, String fromWhere) {
    ...
    try {
      ...
      if (fromWhere.equalsIgnoreCase("author")) SQL_REFERENCES = "SELECT DISTINCT D.SOURCE ";
      if (fromWhere.equalsIgnoreCase("date")) SQL_REFERENCES = "SELECT DISTINCT E.CREATED ";
      if (fromWhere.equalsIgnoreCase("title")) SQL_REFERENCES = "SELECT DISTINCT F.TITLE ";
      if (fromWhere.equalsIgnoreCase("editor")) SQL_REFERENCES = "SELECT DISTINCT D.EDITOR ";
      if (fromWhere.equalsIgnoreCase("publisher")) SQL_REFERENCES = "SELECT DISTINCT G.PUBLISHER ";
