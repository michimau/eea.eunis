<%--
  ...
  - Description : Sites neighborhood search tsv.
--%>
...
<%@ page import="ro.finsiel.eunis.reports.sites.neighborhood.TSVNeighborhoodReport"%>
<jsp:useBean id="formBean" class="ro.finsiel.eunis.search.sites.neighborhood.NeighborhoodBean" scope="request">
...
<%
  try
  {
    String idMainSite = request.getParameter("idsite");
    float radius = Utilities.checkedStringToFloat(request.getParameter("radius"), 0.0F);

    AbstractTSVReport report = new TSVNeighborhoodReport( request.getSession().getId(), idMainSite, radius,  formBean);
...
%>
    <jsp:include page="../tsv-check-size.jsp">
      ...
      <jsp:param name="page_name" value="tsv-sites-size.jsp" />
    </jsp:include>
<%
    }
    else
