...
this.width =  "auto";
this.height = "auto";
this.bodyStyle = 'background-color: #FFFFFF; font-family: "Arial"; font-size: 12px;';
...
//this.stylesheet = serverURL + "/css/eunis.css"; // full URL to stylesheet
...
function editor_generate(objname,userConfig,fullCSSPath) {
  // Default Settings
  ...
  this.stylesheet = fullCSSPath; // full URL to stylesheet
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
      ...
      if (btnName == 'linebreak') { alert("Editor error: 'linebreak' must be in a subgroup by itself, not with other buttons.\n\nEditor not created."); return; }
      if (!btnObj) { alert("Editor error: button '" +btnName+ "' not found in button list when creating the editor for '"+objname+"'.\nPlease make sure you entered the button name correctly.\n\nEditor not created."); return; }
...
function editor_action(button_id) {
  ...
  // Custom3
    ...
    editor_insertHTML(objname, "Add buttons that insert text!");
  }
