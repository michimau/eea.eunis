<%--
  ...
  - Description : 'Pick habitat, show species' function - Display in a popup habitats related to selected species.
--%>
...
<html lang="<%=SessionManager.getCurrentLanguage()%>" xmlns="http://www.w3.org/1999/xhtml" xml:lang="<%=SessionManager.getCurrentLanguage()%>">
  <head>
    ...
    %>
    <title>
      <%=contentManagement.getContent("species_habitats-detail_title", false )%>
    </title>
  ...
  <%
    ...
    List results = new ScientificNameDomain().findHabitatsWithSpecies(new HabitateSearchCriteria(searchAttribute,
...
<body style="background-color:#ffffff">
      <span style="background-color:#EEEEEE">
          <strong><%=contentManagement.getContent("species_habitats-detail_01")%>:</strong>
      </span>
      <br />
      <%=contentManagement.getContent("species_habitats-detail_02")%>:<strong> <%=results.size()%> </strong>
      <%
        if (results==null || results.size() < 1)
        {
      %>
          <strong><%=contentManagement.getContent("species_habitats-detail_03")%>.</strong>
          <br />
      <%
        } else
        {
      %>
       ...
       %>
          ...
          <br />
          <form action="">
            <label for="button" class="noshow"><%=contentManagement.getContent("species_habitats-detail_04")%></label>  
            <input id="button" title="Close window" type=button value="<%=contentManagement.getContent("species_habitats-detail_04")%>" onclick="javascript:window.close()" name="button2" class="inputTextField" />
          </form>
  </body>
</html>