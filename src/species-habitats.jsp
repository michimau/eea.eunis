...
<html lang="<%=SessionManager.getCurrentLanguage()%>" xmlns="http://www.w3.org/1999/xhtml" xml:lang="<%=SessionManager.getCurrentLanguage()%>">
  <head>
    ...
    <!--
     var errInvalidRegion = '<%=cm.cms("species_habitats_26")%>';
  ...
  <div id="content">
    <jsp:include page="header-dynamic.jsp">
      <jsp:param name="location" value="home_location#index.jsp,habitats_location#habitats.jsp,pick_habitat_type_show_species_location" />
          ...
          <td>
            ...
            <div class="grey_rectangle">
              ...
              <input title="<%=cm.cms("scientific_name")%>" id="checkbox4" type="checkbox" name="showScientificName" value="true" disabled="disabled" checked="checked" />
                <label for="checkbox4"><%=cm.cmsText("species_habitats_06")%></label>
              ...
              <input title="<%=cm.cms("habitat_types")%>" id="checkbox5" type="checkbox" name="showHabitats" value="true" checked="checked" disabled="disabled" />
                <label for="checkbox5"><%=cm.cmsText("species_habitats_07")%></label>
        ...
        <tr>
          <td colspan="2">
            ...
            <select id="select1" title="<%=cm.cms("search_attribute")%>" name="searchAttribute" class="inputTextField">
              <option value="<%=HabitateSearchCriteria.SEARCH_NAME%>" selected="selected"><%=cm.cms("species_habitats_09")%></option>
              <option value="<%=HabitateSearchCriteria.SEARCH_CODE%>"><%=cm.cms("species_habitats_10")%></option>
              <option value="<%=HabitateSearchCriteria.SEARCH_LEGAL_INSTRUMENTS%>"><%=cm.cms("species_habitats_11")%></option>
              <option value="<%=HabitateSearchCriteria.SEARCH_COUNTRY%>"><%=cm.cms("species_habitats_12")%></option>
              <option value="<%=HabitateSearchCriteria.SEARCH_REGION%>"><%=cm.cms("species_habitats_13")%></option>
            </select>
            ...
            <select id="select2" title="<%=cm.cms("operator")%>" name="relationOp" class="inputTextField">
              <option value="<%=Utilities.OPERATOR_IS%>"><%=cm.cms("species_habitats_14")%></option>
              <option value="<%=Utilities.OPERATOR_CONTAINS%>"><%=cm.cms("species_habitats_15")%></option>
              <option value="<%=Utilities.OPERATOR_STARTS%>" selected="selected"><%=cm.cms("species_habitats_16")%></option>
            </select>
            ...
            <a title="<%=cm.cms("list_values_link")%>" href="javascript:openHelper('species-habitats-choice.jsp')"><img alt="<%=cm.cms("species_habitats_17")%>" height="18" title="<%=cm.cms("species_habitats_17")%>" src="images/helper/helper.gif" width="11" border="0" /></a>
              <%=cm.cmsTitle("list_values")%>
              <%=cm.cmsAlt("species_habitats_17")%>
          </td>
        </tr>
        <tr>
          <td style="text-align:left;background-color:#EEEEEE">
            <%=cm.cmsText("species_habitats_18")%>: &nbsp;
            <input id="radio1" type="radio" name="database" value="<%=ScientificNameDomain.SEARCH_EUNIS%>" checked="checked"
               title="<%=cm.cms("habitat_type_eunis")%>" /><label for="radio1"><%=cm.cmsText("species_habitats_19")%></label><%=cm.cmsTitle("habitat_type_eunis")%>&nbsp;&nbsp;
            <input id="radio2" type="radio" name="database" value="<%=ScientificNameDomain.SEARCH_ANNEX_I%>"
               title="<%=cm.cms("habitat_type_annexI")%>" /><label for="radio2"><%=cm.cmsText("species_habitats_20")%></label><%=cm.cmsTitle("habitat_type_annexI")%>&nbsp;
            <input id="radio3" type="radio" name="database" value="<%=ScientificNameDomain.SEARCH_BOTH%>"
               title="<%=cm.cms("habitat_type_both")%>" /><label for="radio3"><%=cm.cmsText("species_habitats_21")%><%=cm.cmsTitle("habitat_type_both")%></label>
          </td>
        </tr>
        <tr>
...
%>
          ...
          <br />
          <%=cm.cmsText("species_habitats_24")%>:
          <a title="<%=cm.cms("save_title")%>" href="javascript:composeParameterListForSaveCriteria('<%=request.getParameter("expandSearchCriteria")%>',validateForm(),'species-habitats.jsp','2','eunis',attributesNames,formFieldAttributes,operators,formFieldOperators,booleans,'save-criteria-search.jsp');"><img alt="<%=cm.cms("save_title")%>" border="0" src="images/save.jpg" width="21" height="19" style="vertical-align:middle" /></a>
...
<%=cm.br()%>
<%=cm.cmsMsg("species_habitats_26")%>
<%=cm.br()%>
<%=cm.cmsMsg("species_habitats_27")%>
<%=cm.br()%>
<%=cm.cmsMsg("species_habitats_28")%>
<%=cm.br()%>
<%=cm.cmsMsg("species_habitats_title")%>
<%=cm.br()%>
<%=cm.cmsMsg("species_habitats_09")%>
<%=cm.br()%>
<%=cm.cmsMsg("species_habitats_10")%>
<%=cm.br()%>
<%=cm.cmsMsg("species_habitats_11")%>
<%=cm.br()%>
<%=cm.cmsMsg("species_habitats_12")%>
<%=cm.br()%>
<%=cm.cmsMsg("species_habitats_13")%>
<%=cm.br()%>
<%=cm.cmsMsg("species_habitats_14")%>
<%=cm.br()%>
<%=cm.cmsMsg("species_habitats_15")%>
<%=cm.br()%>
<%=cm.cmsMsg("species_habitats_16")%>
<%=cm.br()%>
