...
public class DesignationsFactsheetActionBean extends AbstractStripesAction {
	...
	public Resolution defaultAction() {
		...
		pageTitle = getContext().getInitParameter("PAGE_TITLE") 
			+ getContentManagement().cms("sites_designations-factsheet_title");
		...
		if(idDesig != null && idGeo != null){
	    	...
	    	if(factsheet != null){
	    		...
	    		// Name of designation
	    	    fromWho = (factsheet.getDescription() != null && !factsheet.getDescription().equalsIgnoreCase("") ? factsheet.getDescription() : "");
	    	    if(fromWho.equalsIgnoreCase(""))
	    	    	fromWho = (factsheet.getDescriptionEn() != null && !factsheet.getDescriptionEn().equalsIgnoreCase("") ? factsheet.getDescriptionEn() : "");
	    	    ...
	    	    if(country != null && country.trim().length() > 0 ){
	    	    	List countries = new Chm62edtCountryDomain().findWhere( "ISO_2L<>'' AND ISO_2L<>'null' AND ISO_2L IS NOT NULL AND SELECTION <> 0 and AREA_NAME_EN ='" + country +  "'" );
