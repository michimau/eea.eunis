...
public class HabitatsFactsheetActionBean extends AbstractStripesAction {
    ...
    private static final String[] tabs = {
        "General information", "Geographical distribution", "Legal instruments",
    ...
    private static final String[][] dbtabs = {
        ...
        { "LEGAL_INSTRUMENTS", "instruments"}, { "HABITATS", "habitats"},
        { "SITES", "sites"}, { "SPECIES", "species"}, { "OTHER", "other"}
    };

    private static final Integer[] dictionary = {
        HabitatsFactsheet.OTHER_INFO_ALTITUDE,
        HabitatsFactsheet.OTHER_INFO_DEPTH, HabitatsFactsheet.OTHER_INFO_CLIMATE,
        ...
        HabitatsFactsheet.OTHER_INFO_LIGHT, HabitatsFactsheet.OTHER_INFO_SPATIAL,
        HabitatsFactsheet.OTHER_INFO_TEMPORAL,
        HabitatsFactsheet.OTHER_INFO_IMPACT, HabitatsFactsheet.OTHER_INFO_USAGE
    };
    ...
    public Resolution defaultAction() {
        ...
        btrail = "eea#" + eeaHome
                + ",home#index.jsp,habitat_types#habitats.jsp,factsheet";
        ...
        if (factsheet.getHabitat() == null) {
            ...
            return new ForwardResolution(
                    "/stripes/habitats-factsheet.layout.jsp");
        }
        ...
        for (int i = 0; i < tabs.length; i++) {
            if (!getContext().getSqlUtilities().TabPageIsEmpy(
                    factsheet.idNatureObject.toString(), "HABITATS",
                    dbtabs[i][0])) {
            ...
            if (factsheet.isEunis()) {
                for (int i = 0; i < dictionary.length; i++) {
                    try {
                        ...
                        String title = factsheet.getOtherInfoDescription(
                                dictionaryType);
                        String SQL = factsheet.getSQLForOtherInfo(dictionaryType);
                        String noElements = getContext().getSqlUtilities().ExecuteSQL(
                                SQL);
    ...
    private void sitesTabActions() {
        ...
        if ((null != sites && !sites.isEmpty())
                || (null != sitesForSubtypes && !sitesForSubtypes.isEmpty())) {
            mapIds = "";
