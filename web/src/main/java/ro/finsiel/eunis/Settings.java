...
public class Settings {
  ...
  public static String getSetting(String name) {
    String ret = "";
    if (null == name) {
      System.out.println("Settings::getSetting(" + name + ") - Warning: Parameter was null. Returning empty string.");
    } else {
      if (settings.containsKey(name)) {
        ret = (String) settings.get(name);
      } else {
        System.out.println("Settings::getSetting(" + name + ") - Warning: Value not found in settings object. Returning empty string.");
        if (!loaded) {
          System.out.println("Settings::getSetting(" + name + ") - Warning: Settings not loaded. Load settings before using this method.");
        }
      }
    }
    return ret;
  ...
  public static void setSetting(String name, String value) {
    if (null == name || null == value) {
      if (null != name && (name.equalsIgnoreCase("PROXY_URL") || name.equalsIgnoreCase("PROXY_PORT")))
      {
        System.out.println("Settings::setSetting(" + name + ", " + value + ") - Warning: null parameters. Not using proxy.");
      } else {
        System.out.println("Settings::setSetting(" + name + ", " + value + ") - Warning: null parameters. Value not loaded.");
      }
      return;
    }
  ...
  public static void loadSettings(HttpServlet servlet) {
    ...
    while (en.hasMoreElements())
    {
      ...
      Settings.setSetting(param, servlet.getServletContext().getInitParameter(param));
