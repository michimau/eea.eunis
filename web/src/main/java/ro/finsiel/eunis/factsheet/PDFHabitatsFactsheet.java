...
public class PDFHabitatsFactsheet {
    ...
    private void getGeneralInformation() throws Exception {
        ...
        if (otherClassifHabitats.size() > 0 || otherHabitats.size() > 0) {
            ...
            for (int j = 0; j < otherClassifHabitats.size(); j++) {
                ...
                cell = new Cell(
                        new Phrase(HabitatsFactsheet.mapHabitatsRelations(otherClassifHabitat.getRelationType()), fontNormal));
                table.addCell(cell);
            }
    ...
    private void getRelatedSites() throws Exception {
        ...
        List sites = new SitesByNatureObjectDomain().findCustom(
                "SELECT C.ID_SITE, C.NAME, C.SOURCE_DB, C.LATITUDE, C.LONGITUDE, E.AREA_NAME_EN " + " FROM CHM62EDT_HABITAT AS A "
