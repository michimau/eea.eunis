...
public class PicturesHelper {
  ...
  public static String findSpeciesByIDObject(String IDObject) {
    ...
    try
    {
      species = new Chm62edtSpeciesDomain().findWhere("ID_SPECIES = " + IDObject);
      if (null != species && species.size() > 0)
      {
        speciesObject = ((Chm62edtSpeciesPersist) species.get(0));
        if (null != speciesObject) ret = speciesObject.getScientificName();
      }
    } catch (Exception _ex) {
  ...
  public static String findSitesByIDObject(String IDObject) {
    ...
    try
    {
      ...
      if (null != species && species.size() > 0)
      {
        ...
        if (null != sitesObject) ret = sitesObject.getName();
      }
    } catch (Exception _ex) {
      _ex.printStackTrace(System.err);
    }
  ...
  public static String findHabitatsByIDObject(String IDObject) {
    ...
    try
    {
      species = new Chm62edtHabitatDomain().findWhere("ID_HABITAT = " + IDObject);
      if (null != species && species.size() > 0)
      {
  ...
  public static boolean deletePicture(String idObject, String natureObjectType, String name, String filename) {
    ...
    if (null != idObject && null != natureObjectType && null != name && null != filename)
    {
      ...
      logger.warn("deletePicture(" + idObject + ", " + natureObjectType + ", "  + name + ", " + filename + ") : One of the parameters was null!");
    }
    return ret;
  }
