...
//
  ...
  public HumanActivityAttributesPersist findHumanActivityAttribute( String attribute, int position ) {
    ...
    try
    {
      List results = new HumanActivityAttributesDomain().findWhere( "LOOKUP_TYPE = 'HUMAN_ACTIVITY' AND ID_SITE='" + getSiteObject().getIdSite() + "' AND C.NAME='" + attribute + "' LIMIT " + position + ", 10 " );
      if ( null != results && results.size() > 0 )
      {
...
//  }
  ...
  public String getSiteType() {
    ...
    try
    {
      results = new Chm62edtSitesAttributesDomain().findWhere( " NAME='TYPE' AND ID_SITE = '" + getSiteObject().getIdSite() + "'" );
      if ( results.size() > 0 )
      {
