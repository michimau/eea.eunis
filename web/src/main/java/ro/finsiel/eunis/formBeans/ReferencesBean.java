...
public class ReferencesBean extends AbstractFormBean {
    ...
    public AbstractSearchCriteria getMainSearchCriteria() {
        ...
        relationOpAuthor = Utilities.checkedStringToInt(this.relationOpAuthor,
                Utilities.OPERATOR_CONTAINS);
        relationOpDate = Utilities.checkedStringToInt(this.relationOpDate,
                Utilities.OPERATOR_IS);
        relationOpTitle = Utilities.checkedStringToInt(this.relationOpTitle,
                Utilities.OPERATOR_CONTAINS);
        relationOpEditor = Utilities.checkedStringToInt(this.relationOpEditor,
                Utilities.OPERATOR_CONTAINS);
        relationOpPublisher = Utilities.checkedStringToInt(
                this.relationOpPublisher, Utilities.OPERATOR_CONTAINS);

        criteria = new ReferencesSearchCriteria(author, relationOpAuthor, date,
                date1, relationOpDate, title, relationOpTitle, publisher,
                relationOpPublisher, editor, relationOpEditor);
        return criteria;
    }
    ...
    public AbstractSearchCriteria[] toSearchCriteria() {
        ...
        // Main Search
        Integer relationOpAuthor = Utilities.checkedStringToInt(
                this.relationOpAuthor, Utilities.OPERATOR_CONTAINS);
        Integer relationOpDate = Utilities.checkedStringToInt(
                this.relationOpDate, Utilities.OPERATOR_IS);
        Integer relationOpTitle = Utilities.checkedStringToInt(
                this.relationOpTitle, Utilities.OPERATOR_CONTAINS);
        Integer relationOpEditor = Utilities.checkedStringToInt(
                this.relationOpEditor, Utilities.OPERATOR_CONTAINS);
        Integer relationOpPublisher = Utilities.checkedStringToInt(
                this.relationOpPublisher, Utilities.OPERATOR_CONTAINS);

        criterias.addElement(
                new ReferencesSearchCriteria(author, relationOpAuthor, date,
                date1, relationOpDate, title, relationOpTitle, publisher,
                relationOpPublisher, editor, relationOpEditor));
        // Search in results criterias
        if (null != criteriaSearch && null != oper && null != criteriaType) {
            for (int i = 0; i < criteriaSearch.length; i++) {
                Integer _criteriaType = Utilities.checkedStringToInt(
                        criteriaType[i],
                        ReferencesSearchCriteria.CRITERIA_AUTHOR);
                Integer _oper = Utilities.checkedStringToInt(oper[i],
                        Utilities.OPERATOR_CONTAINS);

                criterias.addElement(
                        new ReferencesSearchCriteria(criteriaSearch[i],
                        _criteriaType, _oper));
            }
        }
    ...
    public AbstractSortCriteria[] toSortCriteria() {
        ...
        for (int i = 0; i < sort.length; i++) {
            ReferencesSortCriteria criteria = new ReferencesSortCriteria(
                    Utilities.checkedStringToInt(sort[i],
                    AbstractSortCriteria.ASCENDENCY_NONE),
                    Utilities.checkedStringToInt(ascendency[i],
                    AbstractSortCriteria.ASCENDENCY_NONE));
    ...
    public String toURLParam(Vector classFields) {
        ...
        if (null != showAuthor && showAuthor.equalsIgnoreCase("true")) {
            url.append(
                    Utilities.writeURLParameter("showAuthor",
                    AbstractFormBean.SHOW.toString()));
        }
        if (null != showYear && showYear.equalsIgnoreCase("true")) {
            url.append(
                    Utilities.writeURLParameter("showYear",
                    AbstractFormBean.SHOW.toString()));
        }
        if (null != showTitle && showTitle.equalsIgnoreCase("true")) {
            url.append(
                    Utilities.writeURLParameter("showTitle",
                    AbstractFormBean.SHOW.toString()));
        }
        if (null != showEditor && showEditor.equalsIgnoreCase("true")) {
            url.append(
                    Utilities.writeURLParameter("showEditor",
                    AbstractFormBean.SHOW.toString()));
        }
        if (null != showPublisher && showPublisher.equalsIgnoreCase("true")) {
            url.append(
                    Utilities.writeURLParameter("showPublisher",
                    AbstractFormBean.SHOW.toString()));
        }
        if (null != showURL && showURL.equalsIgnoreCase("true")) {
            url.append(
                    Utilities.writeURLParameter("showURL",
                    AbstractFormBean.SHOW.toString()));
        }
        return url.toString();
    }
    ...
    public String toFORMParam(Vector classFields) {
        ...
        if (null != showAuthor && showAuthor.equalsIgnoreCase("true")) {
            form.append(
                    Utilities.writeFormParameter("showAuthor",
                    AbstractFormBean.SHOW.toString()));
        }
        if (null != showYear && showYear.equalsIgnoreCase("true")) {
            form.append(
                    Utilities.writeFormParameter("showYear",
                    AbstractFormBean.SHOW.toString()));
        }
        if (null != showTitle && showTitle.equalsIgnoreCase("true")) {
            form.append(
                    Utilities.writeFormParameter("showTitle",
                    AbstractFormBean.SHOW.toString()));
        }
        if (null != showEditor && showEditor.equalsIgnoreCase("true")) {
            form.append(
                    Utilities.writeFormParameter("showEditor",
                    AbstractFormBean.SHOW.toString()));
        }
        if (null != showPublisher && showPublisher.equalsIgnoreCase("true")) {
            form.append(
                    Utilities.writeFormParameter("showPublisher",
                    AbstractFormBean.SHOW.toString()));
        }
        if (null != showURL && showURL.equalsIgnoreCase("true")) {
            form.append(
                    Utilities.writeFormParameter("showURL",
                    AbstractFormBean.SHOW.toString()));
        }
