...
public class DistributionWrapper {
    ...
    public DistributionWrapper(Integer id) {
        try {
            distribution = new ReportsDistributionStatusDomain().findWhere(
                    "ID_NATURE_OBJECT = " + id
                    + " AND (D.LOOKUP_TYPE ='DISTRIBUTION_STATUS' OR D.LOOKUP_TYPE ='GRID') GROUP BY C.NAME,C.LATITUDE,C.LONGITUDE");
        } catch (Exception _ex) {
