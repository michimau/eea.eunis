...
public class ScientificNameDomain extends AbstractDomain implements Paginable {
  ...
  public void setup() {
    ...
    this.addColumnSpec(new StringColumnSpec("COMMON_NAME", "getCommonName", "setCommonName", DEFAULT_TO_NULL));
    // FROM CHM62EDT_TAXONOMY
    this.addColumnSpec(new StringColumnSpec("NAME", "setTaxonomicNameFamily", "setTaxonomicNameFamily", DEFAULT_TO_NULL));
    this.addColumnSpec(new StringColumnSpec("NAME", "getTaxonomicNameOrder", "setTaxonomicNameOrder", DEFAULT_TO_NULL));
    this.addColumnSpec(new StringColumnSpec("NAME", "taxonomyName", "setTaxonomyName", DEFAULT_TO_NULL));
    this.addColumnSpec(new StringColumnSpec("LEVEL", "taxonomyLevel", "setTaxonomyLevel", DEFAULT_TO_NULL));
    this.addColumnSpec(new StringColumnSpec("TAXONOMY_TREE", "taxonomyTree", "setTaxonomyTree", DEFAULT_TO_NULL));
  ...
  public List getResults(int offsetStart, int pageSize, AbstractSortCriteria[] sortCriteria) throws CriteriaMissingException {
    ...
    String sql = prepareSQL("H.ID_SPECIES, H.ID_NATURE_OBJECT, H.SCIENTIFIC_NAME, H.ID_SPECIES_LINK, " +
            "I.COMMON_NAME, J.NAME, J.NAME, J.NAME, J.LEVEL, J.TAXONOMY_TREE, J.NAME", // To fill the persist correctly
            filterSQL + " GROUP BY H.ID_NATURE_OBJECT", false);
  ...
  private Long _rawCount() throws CriteriaMissingException {
    ...
    String sql = prepareSQL("COUNT(DISTINCT H.ID_NATURE_OBJECT)", filterSQL.toString(), false);
    try {
  ...
  private StringBuffer _prepareWhereSearch() throws CriteriaMissingException {
    ...
    filterSQL.append(Utilities.showEUNISInvalidatedSpecies(" AND H.VALID_NAME", showEUNISInvalidatedSpecies));
    filterSQL.append(" AND ");
    ...
    try {
      ...
      if (sortCriteria.length > 0) {
        ...
        do {
          ...
          HabitateSortCriteria criteria = (HabitateSortCriteria) sortCriteria[i]; // Notice the upcast here
  ...
  public List findHabitatsWithSpecies(HabitateSearchCriteria criteria,
                                      ...
                                      boolean showEUNISInvalidatedSpecies) {
    if (null == criteria) {
      System.out.println("Warning: " + ScientificNameDomain.class.getName() + "::findHabitatsWithSpecies(" + criteria + ", " + "...). One of criterias was null.");
      return new Vector();
    }
    ...
    String sql = prepareSQL("H.ID_SPECIES, H.ID_NATURE_OBJECT, H.SCIENTIFIC_NAME, H.ID_SPECIES_LINK, " +
            ...
            filterSQL + " GROUP BY C.ID_NATURE_OBJECT ORDER BY C.SCIENTIFIC_NAME", true);
    try {
    ...
    for (int i = 0; i < results.size(); i++) {
      ScientificNamePersist habitat = (ScientificNamePersist) results.get(i);
  ...
  public List findPopupLOV(HabitateSearchCriteria criteria,
                           ...
                           boolean showEUNISInvalidatedSpecies) {
    if (null == criteria) {
      System.out.println("Warning: " + ScientificNameDomain.class.getName() + "::findPopupLOV(" + criteria + ", " + "...). One of criterias was null.");
      return new Vector();
    }
    this.searchCriteria = new AbstractSearchCriteria[1];
    this.searchCriteria[0] = criteria;
    ...
    String sql = "";
    // SCIENTIFIC NAME
    if (searchAttribute.intValue() == HabitateSearchCriteria.SEARCH_NAME.intValue()) {
      sql = prepareSQL("H.ID_SPECIES, H.ID_NATURE_OBJECT, H.SCIENTIFIC_NAME, H.ID_SPECIES_LINK, " +
              "C.SCIENTIFIC_NAME, C.SCIENTIFIC_NAME, C.SCIENTIFIC_NAME, C.SCIENTIFIC_NAME,C.SCIENTIFIC_NAME,C.SCIENTIFIC_NAME,C.SCIENTIFIC_NAME",
              filterSQL + " GROUP BY C.ID_NATURE_OBJECT ORDER BY C.SCIENTIFIC_NAME", true);
      try {
        results = new ScientificNameDomain().findCustom(sql);
    ...
    // CODE
    if (searchAttribute.intValue() == HabitateSearchCriteria.SEARCH_CODE.intValue())
    {
      filterSQL = new StringBuffer(Utilities.showEUNISInvalidatedSpecies("AND H.VALID_NAME", showEUNISInvalidatedSpecies));
      filterSQL.append(" AND ");
      if (0 != database.compareTo(ScientificNameDomain.SEARCH_BOTH)) {
        if (0 == database.compareTo(ScientificNameDomain.SEARCH_EUNIS)) {
          filterSQL.append(" C.ID_HABITAT >= 1 and C.ID_HABITAT < 10000 ");
    ...
    // LEGAL INSTRUMENTS
    if (searchAttribute.intValue() == HabitateSearchCriteria.SEARCH_LEGAL_INSTRUMENTS.intValue()) {
    ...
    // COUNTRY
    if (searchAttribute.intValue() == HabitateSearchCriteria.SEARCH_COUNTRY.intValue()) {
    ...
    // REGION
    if (searchAttribute.intValue() == HabitateSearchCriteria.SEARCH_REGION.intValue()) {
      sql = prepareSQL("E.NAME AS COLUMN1", filterSQL.toString() + " GROUP BY E.NAME", true);
      try {
        results = new GenericDomain().findCustom(sql);
      } catch (Exception _ex) {
