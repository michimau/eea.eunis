    ...
    private StringBuffer _prepareWhereSort() {
        ...
        if (sortCriteria.length > 0) {
            for (int j = 0; j < sortCriteria.length; j++) {
                ...
                if (0
                        != LegalSortCriteria.SORT_NONE.compareTo(
                                criteria.getSortCriteria())) { // Do not add if criteria is sort to NOT SORT
                    if (0
                            != LegalSortCriteria.ASCENDENCY_NONE.compareTo(
                                    criteria.getAscendency())) { // Don't add if ascendency is set to none, nasty hacks
                        if (j > 0) {
                            filterSQL.append(", ");
                        }
