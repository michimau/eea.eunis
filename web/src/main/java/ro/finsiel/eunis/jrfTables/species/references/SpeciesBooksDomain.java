  ...
  public List getResults(int offsetStart, int pageSize, AbstractSortCriteria[] sortCriteria) throws CriteriaMissingException {
    ...
    if (pageSize != 0) { // Doesn't make sense for pageSize = 0.
      if (finalResults != null) {
        int i = 0;
        while ((i <= pageSize - 1) && (offsetStart + i < finalResults.size())) {
          if (offsetStart + i < finalResults.size()) results.add(finalResults.get(offsetStart + i));
          i++;
        }
      } else
...
//      if (publisher != null) publisher = publisher.trim().replaceAll("'", "''");
      ...
      condition = (filterSQL.length() > 0 ? " AND " + filterSQL.toString() + " AND " + lineCondition + " " : " AND " + lineCondition + " ");
    } else
      condition = (filterSQL.length() > 0 ? " AND " + filterSQL.toString() : "");

    String SQL = "(SELECT H.ID_SPECIES AS ID, H.ID_SPECIES_LINK AS ID_LINK, H.SCIENTIFIC_NAME AS NAME, " +
            "H.SCIENTIFIC_NAME AS EDITOR, " + new Date(1998, 1, 1).getYear() + " AS DATE, " +
            "H.SCIENTIFIC_NAME AS TITLE, H.SCIENTIFIC_NAME AS PUBLISHER, " +
            "H.SCIENTIFIC_NAME AS URL " +
            ...
            " GROUP BY H.SCIENTIFIC_NAME) " +
            " ORDER BY NAME" +
            " LIMIT 0," + new Integer(Utilities.MAX_POPUP_RESULTS).toString();
  ...
  private Long _rawCount() throws CriteriaMissingException {
    ...
    if (finalResults == null) {
      ...
      String condition = (filterSQL.length() > 0 ? " AND " + filterSQL.toString() : "");
  ...
  private StringBuffer _prepareWhereSearch() throws CriteriaMissingException {
    ...
    filterSQL.append(Utilities.showEUNISInvalidatedSpecies("AND H.VALID_NAME", showInvalidatedSpecies));


    if (searchCriteria.length <= 0) throw new CriteriaMissingException("No criteria set for searching, so I bailed out...");
        ...
        do {
          ...
          if (!criteria.getCriteriaAsString().equals("none")) {// Do not add if criteria is sort to NOT SORT
            if (!criteria.getAscendencyAsString().equals("none")) { // Don't add if ascendency is set to none, nasty hacks
              ...
              if (criteria.getSortCriteria().intValue() == ReferencesSortCriteria.SORT_AUTHOR.intValue()) byWhat = "source";
              if (criteria.getSortCriteria().intValue() == ReferencesSortCriteria.SORT_DATE.intValue()) byWhat = "date";
              if (criteria.getSortCriteria().intValue() == ReferencesSortCriteria.SORT_EDITOR.intValue()) byWhat = "editor";
              if (criteria.getSortCriteria().intValue() == ReferencesSortCriteria.SORT_PUBLISHER.intValue()) byWhat = "publisher";
              if (criteria.getSortCriteria().intValue() == ReferencesSortCriteria.SORT_TITLE.intValue()) byWhat = "title";
              SortListOfPersistentObject sorter = new SortListOfPersistentObject();
              if (criteria.getAscendency().intValue() == AbstractSortCriteria.ASCENDENCY_ASC.intValue())
