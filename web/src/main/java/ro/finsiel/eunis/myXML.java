...
public class myXML implements Serializable {
    ...
    public class Attribute {
        ...
        public void add(String name, Object value) {
            ...
            if (debug) {
                System.out.println(
                        "myXML.Attribute.add(" + name + "," + value + ")");
            }
        }
    ...
    private void dumpelements(String indent, StringWriter out) {
        int i;

        out.write(indent + "<" + tag + Attribute.dumpattributes());
        if (element == null) {
            out.write("/>");
            return;
        }
    ...
    private void readXML(myXML xml, myFileReader in) throws myXMLException, myXMLEncodingException, IOException {
        ...
        while ((c = in.read()) != -1) {
            ...
            case CLOSE_TAG:
                ...
                if (c == '>') { // currently processing endtag like </tag>
                    ...
                    if (endTag.compareTo(xml.getTag()) != 0) {
                        throw new myXMLEncodingException(
                    ...
                    if (debug) {
                        System.out.println(
                                "readXML: state CLOSE_TAG - Element complete...returning");
                    }
                    return;
                } else {
                    sb.append((char) c);
            ...
            case SINGLE_TAG: // Inside <tag/>
                if (debug) {
                    System.out.println(
                            "readXML: state SINGLE_TAG, c=" + (char) c);
                }
                if (xml.getTag() == null) {
                    xml.setTag(sb.toString());
                    sb.setLength(0);
            ...
            case ATTRIBUTE_RVALUE:
                if (debug) {
                    System.out.println(
                            "readXML: state ATTRIBUTE_RVALUE, c=" + (char) c);
                }
                if (c == '"' || c == '\'') {
                    quotec = c;
                    state = QUOTE;
            ...
            case ATTRIBUTE_LVALUE:
                if (debug) {
                    System.out.println(
                            "readXML: state ATTRIBUTE_LVALUE, c=" + (char) c);
                }
            ...
            case ATTRIBUTE_EQUAL:
                if (debug) {
                    System.out.println(
                            "readXML: state ATTRIBUTE_EQUAL, c=" + (char) c);
                }
                if (c == '=') {
                    state = ATTRIBUTE_RVALUE;
                } else if (Character.isWhitespace((char) c)) {
