...
public class CountryBean extends AbstractFormBean {
  ...
  public AbstractSearchCriteria[] toSearchCriteria() {
    ...
    if (null != country && null != region) {
      for (int i = 0; i < country.length; i++) {
        if (null == country[i]) country[i] = "*";
        if (null != country[i] && country[i].equalsIgnoreCase("")) country[i] = "*";
      }

      for (int i = 0; i < region.length; i++) {
        if (null == region[i]) region[i] = "*";
        if (null != region[i] && region[i].equalsIgnoreCase("")) region[i] = "*";
      }
      ...
      if (null != criteriaSearch && null != oper && null != criteriaType) {
        ...
        for (int i = 0; i < criteriaSearch.length; i++) {
          Integer _criteriaType = Utilities.checkedStringToInt(criteriaType[i], CountrySearchCriteria.CRITERIA_SCIENTIFIC_NAME);
  ...
  public String toURLParam(Vector classFields) {
    ...
    if (null != showLevel && showLevel.equalsIgnoreCase("true")) url.append(Utilities.writeURLParameter("showLevel", CountryBean.SHOW));
    if (null != showCode && showCode.equalsIgnoreCase("true")) url.append(Utilities.writeURLParameter("showCode", CountryBean.SHOW));
    if (null != showScientificName && showScientificName.equalsIgnoreCase("true")) url.append(Utilities.writeURLParameter("showScientificName", CountryBean.SHOW));
    if (null != showVernacularName && showVernacularName.equalsIgnoreCase("true")) url.append(Utilities.writeURLParameter("showVernacularName", CountryBean.SHOW));
    return url.toString();
  }
  ...
  public String toFORMParam(Vector classFields) {
    ...
    // Write columns to be displayed
    if (null != showLevel && showLevel.equalsIgnoreCase("true")) form.append(Utilities.writeFormParameter("showLevel", CountryBean.SHOW));
    if (null != showCode && showCode.equalsIgnoreCase("true")) form.append(Utilities.writeFormParameter("showCode", CountryBean.SHOW));
    if (null != showScientificName && showScientificName.equalsIgnoreCase("true")) form.append(Utilities.writeFormParameter("showScientificName", CountryBean.SHOW));
    if (null != showVernacularName && showVernacularName.equalsIgnoreCase("true")) form.append(Utilities.writeFormParameter("showVernacularName", CountryBean.SHOW));
