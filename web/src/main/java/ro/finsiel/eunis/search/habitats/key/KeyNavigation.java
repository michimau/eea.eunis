...
public class KeyNavigation {
    ...
    public List findQuestionAnswers(int level, String idQuestion, Integer idPage) {
        ...
        try {
            result = new Chm62edtHabitatKeyNavigationDomain().findWhere(
                    "PAGE_LEVEL=" + level + " AND ID_QUESTION='" + idQuestion + "' AND ID_PAGE=" + idPage);
        } catch (Exception _ex) {
            _ex.printStackTrace(System.err);
        } finally {
            if (null == result) {
     ...
     */
    public static void main(String[] args) {// System.out.println(new KeyNavigation().fixAdditionalInfo("A (part)"));
    }
}
