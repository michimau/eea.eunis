...
public class CoordinatesSortCriteria extends AbstractSortCriteria {
    ...
    public CoordinatesSortCriteria(Integer sortCriteria, Integer ascendency) {
        ...
        possibleSorts.put(SORT_LAT,
                "IF(A.LAT_NS='N',IF(A.LAT_DEG IS NULL,0,A.LAT_DEG)*3600 + IF(A.LAT_MIN IS NULL,0,A.LAT_MIN)*60 + IF(A.LAT_SEC IS NULL,0,A.LAT_SEC),(-1)*(IF(A.LAT_DEG IS NULL,0,A.LAT_DEG)*3600 + IF(A.LAT_MIN IS NULL,0,A.LAT_MIN)*60 + IF(A.LAT_SEC IS NULL,0,A.LAT_SEC)))");
        possibleSorts.put(SORT_LONG,
                "IF(A.LONG_EW='E',IF(A.LONG_DEG IS NULL,0,A.LONG_DEG)*3600 +IF(A.LONG_MIN IS NULL,0,A.LONG_MIN)*60 + IF(A.LONG_SEC IS NULL,0,A.LONG_SEC),(-1)*(IF(A.LONG_DEG IS NULL,0,A.LONG_DEG) *3600 + IF(A.LONG_MIN IS NULL,0,A.LONG_MIN)*60 + IF(A.LONG_SEC IS NULL,0,A.LONG_SEC)))");
