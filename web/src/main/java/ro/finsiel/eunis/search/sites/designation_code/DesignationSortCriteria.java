...
public class DesignationSortCriteria extends AbstractSortCriteria {
    ...
    public DesignationSortCriteria(Integer sortCriteria, Integer ascendency) {
        ...
        possibleSorts.put(SORT_LAT,
                "IF(C.LAT_NS='N',IF(C.LAT_DEG IS NULL,0,C.LAT_DEG)*3600 + IF(C.LAT_MIN IS NULL,0,C.LAT_MIN)*60 + IF(C.LAT_SEC IS NULL,0,C.LAT_SEC),(-1)*(IF(C.LAT_DEG IS NULL,0,C.LAT_DEG)*3600 + IF(C.LAT_MIN IS NULL,0,C.LAT_MIN)*60 + IF(C.LAT_SEC IS NULL,0,C.LAT_SEC)))"); // Order
        possibleSorts.put(
                SORT_LONG,
                "IF(C.LONG_EW='E',IF(C.LONG_DEG IS NULL,0,C.LONG_DEG)*3600 +IF(C.LONG_MIN IS NULL,0,C.LONG_MIN)*60 + IF(C.LONG_SEC IS NULL,0,C.LONG_SEC),(-1)*(IF(C.LONG_DEG IS NULL,0,C.LONG_DEG) *3600 + IF(C.LONG_MIN IS NULL,0,C.LONG_MIN)*60 + IF(C.LONG_SEC IS NULL,0,C.LONG_SEC)))"); // Order
        possibleSorts.put(
                SORT_YEAR,
