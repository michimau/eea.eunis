...
/**
 * Form bean used for sites->year search.
 * @author finsiel
 */
public class YearBean extends SitesFormBean {
    ...
    public AbstractSearchCriteria getMainSearchCriteria() {
        ...
        if (null != searchString || (null != searchStringMin && null != searchStringMax)) {
            ...
            criteria = new YearSearchCriteria(relationOp, searchString, searchStringMin, searchStringMax, country);
        }
        return criteria;
    }
    ...
    public AbstractSearchCriteria[] toSearchCriteria() {
        ...
        if (null != searchString || (null != searchStringMin && null != searchStringMax)) {
            if (null != relationOp && relationOp.length() > 0) {
                ...
                relationOp = YearSearchCriteria.SEARCH_LENGTH.toString();
            }
            Integer relationOp = Utilities.checkedStringToInt(this.relationOp, Utilities.OPERATOR_CONTAINS);
            YearSearchCriteria _criteria = new YearSearchCriteria(relationOp, searchString, searchStringMin, searchStringMax,
                    country);
        ...
        if (null != criteriaSearch && null != criteriaType && null != oper) {
            for (int i = 0; i < criteriaSearch.length; i++) {
                ...
                criterias.addElement(new YearSearchCriteria(criteriaSearch[i], _criteriaType, _oper));
            }
        }
        YearSearchCriteria[] ret = new YearSearchCriteria[criterias.size()];

        for (int i = 0; i < ret.length; i++) {
            ret[i] = (YearSearchCriteria) criterias.get(i);
        }
    ...
    public AbstractSortCriteria[] toSortCriteria() {
        ...
        for (int i = 0; i < sort.length; i++) {
            YearSortCriteria criteria = new YearSortCriteria(Utilities.checkedStringToInt(sort[i], YearSortCriteria.ASCENDENCY_NONE),
                    Utilities.checkedStringToInt(ascendency[i], YearSortCriteria.ASCENDENCY_NONE));
