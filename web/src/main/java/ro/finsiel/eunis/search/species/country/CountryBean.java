...
public final class CountryBean extends AbstractFormBean {
  ...
  public AbstractSearchCriteria[] toSearchCriteria() {
    if (null == country || null == region) return new AbstractSearchCriteria[0];
    Vector criterias = new Vector();
    for (int i = 0; i < country.length; i++) {
      if (country[i].equals("any")) {
        CountrySearchCriteria cCriteria = new CountrySearchCriteria(country[i], region[i], countryName, regionName, CountrySearchCriteria.CRITERIA_REGION_GEOSCOPE);
        criterias.addElement(cCriteria);
      }
      if (region[i].equals("any")) {
        CountrySearchCriteria rCriteria = new CountrySearchCriteria(country[i], region[i], countryName, regionName, CountrySearchCriteria.CRITERIA_COUNTRY_GEOSCOPE);
        criterias.addElement(rCriteria);
      }
      if (!country[i].equalsIgnoreCase("any") && (!region[i].equalsIgnoreCase("any"))) {
        CountrySearchCriteria rCriteria = new CountrySearchCriteria(country[i], region[i], countryName, regionName, CountrySearchCriteria.CRITERIA_BOTH_COUNTRY_REGION);
        criterias.addElement(rCriteria);
      }
    }
    ...
    for (int i = 0; i < ret.length; i++) ret[i] = (CountrySearchCriteria) criterias.get(i);
