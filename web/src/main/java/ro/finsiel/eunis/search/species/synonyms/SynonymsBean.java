  ...
  public AbstractSearchCriteria[] toSearchCriteria() {
    ...
    if (null != criteriaSearch && null != criteriaType && null != oper) {
      for (int i = 0; i < criteriaSearch.length; i++) {
        Integer _criteriaType = Utilities.checkedStringToInt(criteriaType[i], SynonymsSearchCriteria.CRITERIA_SCIENTIFIC_NAME);
    ...
    for (int i = 0; i < ret.length; i++) ret[i] = (SynonymsSearchCriteria) criterias.get(i);
    return ret;
  }
